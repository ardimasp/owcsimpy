...
class HumanWithActivity(object):
...
if __name__ == "__main__":
    ...
    human = HumanWithActivity(loc=np.array([2.5,2.5]),direction=np.deg2rad(0),
        reflectivities={'skin':0.9,'shirt':0.1,'furniture':1}, activity='reading')
    
    planes = []
    planes.append(human.head.getPartition(Ps=3))
    planes.append(human.body.getPartition(Ps=2))
    planes.append(human.leg.getPartition(Ps=4))
    planes = list(flatten(planes))

    # for plane in planes:
    #     print(plane.reflectivity)
    ...
    # fig,ax = draw(models3d=human,figsize=(6,6),azim=-100,elev=25,xlim=[0,3],ylim=[0,3],zlim=[0,3]);

    fig,ax = draw(figure=fig,axes=ax,planes=planes,
        alphas=[plane.reflectivity for plane in planes], enablevect=False);
        # alphas=[0.5*plane.reflectivity for plane in planes], lengths=0.1);
